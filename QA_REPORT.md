...
| DATA-002 | ACCEPTED LIMITATION | Паспорт `RENT_RATES`/`APPRECIATION_BY_CITY` не найден: поля source/as-of/owner/SLA честно оставлены неизвестными, интерфейс предупреждает пользователя, документация назначает заполнение владельцу продукта |
...
Подробная матрица «первопричина → файл → тест → expected/actual → local/prod →
visual evidence» для каждого ID: `qa_evidence/finding-closure-matrix.md`.
